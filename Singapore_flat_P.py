...
data = pd.read_csv(P)
X = data[["year","town","no_of_rooms","floor_area_sqm","lease_commence_year"]]
Y = data[["resale_price"]]
...
with tab3:
    ...
    if A is not None and B is not None and C is not None and D is not None and E is not None:
       if st.button("CLICK HERE TO PREDICT"):
         W = Prediction.predict([[A,B,C,D,E]]) 
        #  st.write(W)
...
with tab2:
  ...
  st.write("Before proceeding with the predictive analysis, it's imperative to familiarize yourself with the five input boxes integral to the model's functionality. In the first input box, you'll specify the target year for the resale prediction, indicating the year in which you plan to make the purchase. The second input box introduces the concept of town selection, and I will provide a list of towns shortly. To designate your preferred town, input '0' for the first town, '1' for the second town, and increment the number accordingly for subsequent choices.The third input box, room type, allows you to customize your prediction based on the number of rooms in the flat. Input '1' for a single room, '2' for two rooms, and continue this pattern to accurately represent the configuration of the flat you are considering,And there are two changes: 6' for exclusive rooms and 7 for multi-generation rooms.  . The fourth input box pertains to the floor area in square meters, where you'll input the specific floor area of the flat under consideration.Lastly, the fifth input box is dedicated to the lease commencement year, enabling you to input the year in which the lease commenced for the flat you are evaluating. This information is pivotal in the overall estimation process. In summary, these interactive input boxes offer a granular and personalized approach to resale price prediction, factoring in critical elements such as target year, town selection, room type, floor area, and lease commencement year.(TOWN:'ANG MO KIO', 'BEDOK', 'BISHAN', 'BUKIT BATOK', 'BUKIT MERAH', 'BUKIT PANJANG', 'BUKIT TIMAH', 'CENTRAL AREA', 'CHOA CHU KANG', 'CLEMENTI', 'GEYLANG', 'HOUGANG', 'JURONG EAST', 'JURONG WEST', 'KALLANG/WHAMPOA', 'LIM CHU KANG', 'MARINE PARADE', 'PASIR RIS', 'PUNGGOL', 'QUEENSTOWN', 'SEMBAWANG', 'SENGKANG', 'SERANGOON', 'TAMPINES', 'TOA PAYOH', 'WOODLANDS', 'YISHUN')")
...
with tab5:
  ...
  st.markdown("[CLICK HERE TO SEE PREPROCESSING WORK OF THIS MODEL](https://colab.research.google.com/drive/1c2zYApjTSGyjsq6lt07yDy3LGagLBI8w)")
